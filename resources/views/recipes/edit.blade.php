...
@section('content')
    ...
    <form action="/recipes/{{ $recipe->id }}" method="POST" enctype="multipart/form-data">
        ...
        @method('PUT')
        <div class="mb-3">
            <h1 class="text-center fw-bold shadow-sm bg-body rounded">Edytuj przepis</h1>
        </div>
        <div class="mb-3">
            <label for="image" class="form-label h3">Zdjęcie dania</label>
            <input type="file" id="image" class="form-control" name="image" placeholder="Wyślij zdjęcie...">
        </div>
        ...
        <input type="submit" name="submit" id="submit" class="btn btn-info" value="Edytuj przepis!" />
    </form>
